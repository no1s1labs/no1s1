...
def access(userqr):
  ...
  txn_dict = AppContract.functions.checkAccess(userqr).buildTransaction({
        'chainId': 4,
        'from':account,
        'gas': 18600000,
        #'gasLimit':6994000,
...
def activity(distanceChanged,userqr):
  ...
  #nonce = w3.eth.getTransactionCount(account)
  nonce_activity = w3.eth.getTransactionCount(account)
  #print("nonce is",nonce_activity)
  #Calling checkaccess function to confirm the entry of a user
...
def exit(_dooropened,_actualDuration,_userkey):
  ...
  #Calling checkaccess function to confirm the entry of a user
  txn_dict = AppContract.functions.exit(_dooropened,_actualDuration,_userkey).buildTransaction({
        'chainId': 4,
        'from':account,
        'gas': 18600000,
        #'gasLimit':6994000,
